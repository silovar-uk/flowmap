...
- 囲みの見出しと図形の間、図形同士の縦横間隔を拡張
- 「整える」実行時は、図形間と囲み間に誤操作しにくい間隔を確保
- 「全体」表示では、図全体が画面端へ張り付かない外周余白を確保
- ミニマップ、操作ボタン、ヒントを画面端から少し離して配置
- 狭い画面では余白を縮め、キャンバス領域を優先

### 追加と編集

- ボードの空白をダブルクリック：その場に「処理」を追加し、すぐ入力
- 上部の「図形」：開始／終了、処理、判断、入出力、書類を選んで追加
- 図形をダブルクリック／選択してEnter：その場でタイトル編集
